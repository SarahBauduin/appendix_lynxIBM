...
#    `functionName` is a function's name in your module (e.g., `lynxIBMEvent1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/Bauduin/Documents/GitHub/appendix_lynxIBM/module/lynxIBM/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("lynxIBM")
  path <- list(modulePath = "C:/Users/Bauduin/Documents/GitHub/appendix_lynxIBM/module",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    lynxIBM = list(.saveInitialTime = NA)
  )
  ...
  if (exists("lynxIBMEvent1", envir = .GlobalEnv)) {
    simOutput <- lynxIBMEvent1(mySim)
  } else {
  ...
  if (exists("lynxIBMEvent2", envir = .GlobalEnv)) {
    simOutput <- lynxIBMEvent2(mySim)
  } else {
